...
const vacations = calendar.restDays.map(it => getDatesInRange(it.from, it.to)).flat(1).map(it => formatWithDash(date.formatDate(it, 'YYYY/MM/DD')))
...
watch(() => selectedDate, () => {
          ...
          if (selectedDate == null) {
            ...
            if (selectedDate == formatWithDash(calendar.beginningOfYear)) {
              ...
              title = title!! + calendar.restDays.filter(it => isDateInRange(selectedDate!!, it.from, it.to))
